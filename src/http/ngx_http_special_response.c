...
static u_char ngx_http_801_tail[] = " ";
...
static char ngx_http_error_801_page[] =
"{'code':801, 'message':'too many access', 'data':''}" 
;
...
#define NGX_HTTP_OFF_8XX   (NGX_HTTP_LAST_5XX - 500 + NGX_HTTP_OFF_5XX)
    ngx_string(ngx_http_error_801_page)
#define NGX_HTTP_LAST_8XX  802
};
...
ngx_http_special_response_handler(ngx_http_request_t *r, ngx_int_t error)
{
    ...
    //} else if (error == 801) {    // 自己加的码
    } else if (error > NGX_HTTP_LAST_5XX && error < NGX_HTTP_LAST_8XX) {    // 自己加的码
        err = error - NGX_HTTP_LAST_8XX + NGX_HTTP_OFF_8XX + 6 + (error - 800);
    } else {
    ...
    ngx_http_core_loc_conf_t *clcf, ngx_uint_t err)
{
    ...
    if (err >= (NGX_HTTP_OFF_8XX+6)) {
        len = sizeof(ngx_http_801_tail) - 1;
        tail = ngx_http_801_tail;
    ...
    printf("err err err err is %d, err data is %s, err len is %d\n", (int)err, ngx_http_error_pages[err].data, (int)ngx_http_error_pages[err].len);
    if (ngx_http_error_pages[err].len) {
            ...
            && err >= NGX_HTTP_OFF_4XX)
        {
            ...
            msie_padding = 1;
            printf("xxxxxxxxxxxyyyyyyyyyyyyzzzzzzzzzzzzz\n");
        }
    ...
    b->last_in_chain = 1;
    printf("jiajieshi jiajieshi jiajieshi\n");
