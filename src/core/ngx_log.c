...
ngx_int_t
ngx_log_open_default(ngx_cycle_t *cycle)    // 在这个函数里，并没有打开error log
{
    ngx_log_t         *log;
    ...
    printf("=====error_log is %s\n", error_log.data);
    log->file = ngx_conf_open_file(cycle, &error_log);   // 其实这一行并没有打开error_log，只是将error log的全路径放到error_log里面了，并且把error log的路径放到了cycle->open_files里面，并且返回给了log->file
    if (log->file == NULL) {
        return NGX_ERROR;
    }
