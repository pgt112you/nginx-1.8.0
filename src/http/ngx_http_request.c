...
{
    printf("honda honda honda honda\n");
    ngx_uint_t              i;
    ngx_event_t            *rev;
    ...
    // 到这里c->listening->servers->addrs->conf->default_server->ctx 已经不是空的了
    if (port->naddrs > 1) {
        ...
        default: /* AF_INET */
            ...
            ///* test */   // 到这里c->listening->servers->addrs->conf->default_server->ctx就不是空的了
            //char *ss = "11";
            //sprintf(ss, "lllllll");
            ///* end test */
            break;
    ...
    printf("changan changan changan !!!!\n");
    if (ngx_handle_read_event(rev, 0) != NGX_OK) {
        ngx_http_close_connection(c);
        return;
    }
    printf("changan11 changan11 changan11 !!!!\n");
}
...
ngx_http_create_request(ngx_connection_t *c)
{
    ...
    printf("xxxxxxxxxxxxxx addr is %p\n", r->srv_conf);
...
{
    printf(">>>>>>in ngx_http_finalize_request    %d\n", (int)rc);
    ngx_connection_t          *c;
        ...
        || c->error)
    {
        ...
        printf("aaaaabbbbbcccdddddeeeee\n");
        ...
        || rc == NGX_HTTP_NO_CONTENT)
    {
        ...
        c->read->handler = ngx_http_request_handler;
        c->write->handler = ngx_http_request_handler;
        printf("lalalallala---------------lalalalalala\n");
